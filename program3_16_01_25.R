...
# What is highest score of tendulkar?
match_stat[which(match_stat$name=="Tendulkar"),4]

# Display the name and the average of the player who is having maximum highestscore.
match_stat[which(match_stat$highestscore==max(match_stat$highestscore)),c(1,5)]

# Display the name,matches,innings and average of the players having score above 250.
match_stat[which(match_stat$highestscore>=250),c(1,2,3,5)]

# working on dataset internal
data()
